...
def gnn_first_layer_aggregation(G, features_tensor, activation, device, community_cache):
    ...
    for v in G.nodes():
        ...
        if len(local_communities) == 0:
            new_features[v] = (torch.cat([feats_v, feats_v], dim=-1))
            continue
...
def gnn_second_layer_aggregation(G, features_tensor, activation, device, community_cache):
    ...
    for v in G.nodes():
        neighbors = list(G.neighbors(v))
        feats_v = features_tensor[v]
        local_communities = community_cache[v]  # ← 邻居连通子图的分量，不变
        ...
        for community in local_communities:
            idx = torch.tensor(list(community), dtype=torch.long, device=device)
            feats_stack = features_tensor[idx]  # ← 用的是当前层输入特征
            avg_pool = feats_stack.mean(dim=0)
            community_means.append(avg_pool)

        centers = torch.stack(community_means, dim=0)  # shape = (k, D)
        neighbor_ids = torch.tensor(neighbors, dtype=torch.long, device=device)
        neighbor_feats = features_tensor[neighbor_ids]  # ← 当前层输入特征

        # 聚类：邻居特征 → 上述中心点
        dists = torch.cdist(neighbor_feats, centers, p=2)  # (num_neighbors, num_clusters)
        cluster_labels = torch.argmin(dists, dim=1)  # 最近中心分配
...
if __name__ == "__main__":
    ...
    df.to_excel("1OURS_results6_kmeans0_layer6_cora.xlsx", index=False)
