...
# 第二层聚合（邻居聚类 + scatter 聚合）
def safe_to_device(tensor, device):
    try:
        return tensor.to(device)
    except OutOfMemoryError:
        print("⚠️ 显存不足，自动转移张量到 CPU 继续执行。")
        return tensor.cpu()
...
def gnn_second_layer_aggregation(G, features_tensor, activation, device, community_cache):
    ...
    N, D = features_tensor.size()
    new_features = torch.zeros((N, 2 * D), device=device if use_gpu else 'cpu')
    ...
    for v in G.nodes():
        try:
            ...
            community_means = []
            for community in local_communities:
                idx = torch.tensor(list(community), dtype=torch.long)
                idx = safe_to_device(idx, device if use_gpu else 'cpu')
                feats_stack = features_tensor[idx]
                avg_pool = feats_stack.mean(dim=0)
                community_means.append(avg_pool)

            centers = torch.stack(community_means, dim=0)
            neighbor_ids = torch.tensor(neighbors, dtype=torch.long)
            neighbor_ids = safe_to_device(neighbor_ids, device if use_gpu else 'cpu')
            neighbor_feats = features_tensor[neighbor_ids]

            # 计算邻居到中心的欧氏距离
            dists = torch.cdist(neighbor_feats, centers, p=2)
            ...
            print(f"⚠️ 节点 {v} 处理时显存不足，强制退回 CPU 执行")
            ...
            # 重新处理当前节点
            feats_v = features_tensor[v].cpu()
            neighbor_ids = torch.tensor(neighbors, dtype=torch.long)
            neighbor_feats = features_tensor[neighbor_ids].cpu()

            community_means = []
            for community in community_cache[v]:
                idx = torch.tensor(list(community), dtype=torch.long)
                feats_stack = features_tensor[idx].cpu()
                avg_pool = feats_stack.mean(dim=0)
                community_means.append(avg_pool)
    ...
    return new_features.to(device if use_gpu else 'cpu')
...
if __name__ == "__main__":
    ...
    df.to_excel("1OURS_results6_kmeans_repeat_layer6_cora.xlsx", index=False)
