...
def gnn_second_layer_aggregation(G, features_tensor, activation, device, community_cache):
    ...
    for v in G.nodes():
        ...
        neighbor_feats = features_tensor[torch.tensor(neighbors, dtype=torch.long, device=device)]

        if len(neighbors) <= 2:
            avg_pool = neighbor_feats.mean(dim=0)
            combined = torch.cat([feats_v, avg_pool], dim=-1)
            new_features[v] = activation(combined)
            continue

        # 将邻居特征搬到 CPU 上执行 DBSCAN（sklearn 不支持 GPU）
        neighbor_feats_cpu = neighbor_feats.detach().cpu().numpy()
        clustering = DBSCAN(eps=0.5, min_samples=2).fit(neighbor_feats_cpu)
        labels = clustering.labels_

        if np.all(labels == -1):  # 全为噪声
            avg_pool = neighbor_feats.mean(dim=0)
            combined = torch.cat([feats_v, avg_pool], dim=-1)
            new_features[v] = activation(combined)
            continue
        ...
        for lbl in np.unique(labels):
            ...
            cluster_feats = neighbor_feats[idx]
            avg_pool = cluster_feats.mean(dim=0)
            agg_list.append(torch.cat([feats_v, avg_pool], dim=-1))
...
if __name__ == "__main__":
    ...
    # plt.legend()
    plt.title("OURS_kmeans_dbscan Node Embedding Visualization (Cora, t-SNE)")
    plt.xlabel("t-SNE Dim 1")
    plt.ylabel("t-SNE Dim 2")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("1ours_dbscan.png", dpi=300)
    plt.show()
